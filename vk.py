...
# Modified by:  Albert De La Fuente (Nov 2012)
...
def scrapertoolscache_page(url,post=None,headers=[['User-Agent', 'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.6; es-ES; rv:1.9.2.12) Gecko/20101026 Firefox/3.6.12']],modo_cache=CACHE_ACTIVA, timeout=socket.getdefaulttimeout()):
...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    ...
    data = scrapertoolscache_page(page_url.replace("amp;",""))
    videourl = ""
    ...
    #scrapertoolsprintMatches(matches)
    ...
    if len(matches)>0:
        ...
        data2 = data.replace("\\","")
        video_host = scrapertoolsget_match(data2,'"host":"([^"]+)"')
        video_uid = scrapertoolsget_match(data2,'"uid":"([^"]+)"')
        video_vtag = scrapertoolsget_match(data2,'"vtag":"([^"]+)"')
        video_no_flv = scrapertoolsget_match(data2,'"no_flv":([0-9]+)')
        video_max_hd = scrapertoolsget_match(data2,'"hd":([0-9]+)')
